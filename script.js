...
// Theme Count Functionality
let themeCountInterval;
...
async function fetchThemeCounts() {
    try {
        ...
        if (data.success) {
            updateThemeDisplay(data.themes, data.total_registrations);
        } else {
            console.error('Failed to fetch theme counts:', data.error);
        }
    } catch (error) {
...
function updateThemeDisplay(themes, totalRegistrations) {
    ...
    // Update individual theme counts
    Object.keys(themes).forEach(themeKey => {
        const theme = themes[themeKey];
        const countElement = document.getElementById(`count-${themeKey}`);
        const statusElement = document.getElementById(`status-${themeKey}`);
        const themeCard = document.querySelector(`[data-theme="${themeKey}"]`);
        
        if (countElement) {
            countElement.textContent = theme.count;
        ...
        if (statusElement && themeCard) {
            if (theme.available) {
                statusElement.innerHTML = '<span class="status-available">Available</span>';
                themeCard.classList.remove('theme-full');
                themeCard.classList.add('theme-available');
            } else {
                statusElement.innerHTML = '<span class="status-full">Full (100/100)</span>';
                themeCard.classList.remove('theme-available');
                themeCard.classList.add('theme-full');
            }
        }
    });
    ...
    if (refreshNote) {
        refreshNote.textContent = `Last updated: ${timeString} • Auto-refresh every 30 seconds`;
    }
}

function refreshThemeCounts() {
    const refreshButton = document.querySelector('.theme-refresh button');
    if (refreshButton) {
        refreshButton.disabled = true;
        refreshButton.innerHTML = '<span class="refresh-icon spinning">🔄</span> Refreshing...';
    }
    
    fetchThemeCounts().finally(() => {
        if (refreshButton) {
            refreshButton.disabled = false;
            refreshButton.innerHTML = '<span class="refresh-icon">🔄</span> Refresh Counts';
        }
    });
}

function startThemeCountUpdates() {
    // Initial fetch
    fetchThemeCounts();
    
    // Set up auto-refresh every 30 seconds
    themeCountInterval = setInterval(fetchThemeCounts, 30000);
}

function stopThemeCountUpdates() {
    if (themeCountInterval) {
        clearInterval(themeCountInterval);
        themeCountInterval = null;
    }
}

// Initialize theme counts when page loads
document.addEventListener('DOMContentLoaded', function() {
    // Only start theme count updates if we're on a page with themes
    if (document.getElementById('totalRegistrations')) {
        startThemeCountUpdates();
    }
});
...
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        stopThemeCountUpdates();
    } else if (document.getElementById('totalRegistrations')) {
        startThemeCountUpdates();
    }
});

// Make refreshThemeCounts available globally
window.refreshThemeCounts = refreshThemeCounts;